...
export default function VolleyballDetails({ match, onBack }) {
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 mb-2 text-gray-700 text-base">
        <div className="flex flex-row justify-between items-center w-full">
          <span className="flex flex-col">
            <span>
              Sport: <span className="font-semibold text-blue-700">{match.sportName || 'Volleyball'}</span>
            </span>
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2">
          {match.sets.map((set, i) => {
            ...
            return (
              ...
              >
                <span className="font-semibold text-gray-700 mb-1">Set {i + 1}</span>
                <span>
                  <span className={`text-blue-900 font-bold ${homeWon ? 'bg-green-100 rounded px-2' : ''}`}>
                    {set.home}
                  </span>
                  <span className="mx-1 text-gray-500">-</span>
                  <span className={`text-red-900 font-bold ${awayWon ? 'bg-green-100 rounded px-2' : ''}`}>
                    {set.away}
                  </span>
                </span>
              </div>
